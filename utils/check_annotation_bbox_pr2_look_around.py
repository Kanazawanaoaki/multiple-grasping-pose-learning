...
def dataset_generate(rgb_img,label,roi_dimension):
    ...
    suffix = "frame%04d.jpg" %image_counter
    save_check_rgb = save_data_path + "/check/frame%04d.jpg" %image_counter
    path = "does not matter"
    image_counter = image_counter + 1
    ...
    cv2.imwrite(save_check_rgb,check_img)
    boundingbox =  OrderedDict([
        ("xmin", roi_min_x),
        ("ymin", roi_min_y),
        ("xmax", roi_max_x),
            ...
            for imgfile in glob.glob(os.path.join(datafile, '*jpg')):
                ...
                if suffix == 'rgb.jpg':
                    ...
                    for i in range(len(contours)):    
                        ...
                        # # max filter
                        # if len(contour_points) > len(max_contour):
                        #     max_contour = contour_points
                        # len filter
                    ...
                    obj_names.append(imgfile)
                    obj_roi_dimensions.append(roi_dimension)
                    obj_rgb_imgs.append(rgb_img)
                    obj_labels.append(str(cnt))

        # obj filter
        roi_array = np.array(obj_roi_dimensions)
        roi_mean = np.mean(roi_array, axis=0)
        roi_std = np.std(roi_array, axis=0)
        for i in range(len(obj_roi_dimensions)):
            if roi_std[2] > 20:
                if roi_array[i][2] < roi_mean[2] - roi_std[2]*0.5:
                    print("excluding {}".format(obj_names[i]))
                    continue
            if roi_std[3] > 20:
                if roi_array[i][3] < roi_mean[3] - roi_std[3]*0.5:
                    print("excluding {}".format(obj_names[i]))
                    continue
            dataset_generate(obj_rgb_imgs[i],obj_labels[i],obj_roi_dimensions[i])
